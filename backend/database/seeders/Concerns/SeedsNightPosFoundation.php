...
trait SeedsNightPosFoundation
{
    ...
    public function seedNightPosFoundation(): void
    {
        ...
        $permissions = collect([
            ...
            ['name' => 'Gestionar multas de liquidación', 'slug' => 'settlements.fines.manage'],
            ...
            ['name' => 'Mesas de servicio', 'slug' => 'settings.service_tables'],
            ['name' => 'Gestionar mesas', 'slug' => 'settings.service_tables.manage'],
            ['name' => 'Asignar mesas a garzones', 'slug' => 'settings.waiter_assignments'],
            ['name' => 'Gestionar asignación mesas', 'slug' => 'settings.waiter_assignments.manage'],
            ...
            ['name' => 'Impresoras', 'slug' => 'settings.printers'],
            ['name' => 'Gestionar impresoras', 'slug' => 'settings.printers.manage'],
            ['name' => 'Reimprimir tickets', 'slug' => 'printing.reprint'],
            ...
            ['name' => 'Modo garzón — mis mesas', 'slug' => 'waiter.my_tables'],
        ...
        $roleOwner->permissions()->sync(
            $permissions->whereIn('slug', TenantDefaultRolePermissions::tenantOwner())->pluck('id')
        );

        $roleCashier->permissions()->sync($permissions->whereIn('slug', TenantDefaultRolePermissions::cashier())->pluck('id'));

        $roleCashierSenior->permissions()->sync($permissions->whereIn('slug', TenantDefaultRolePermissions::cashierSenior())->pluck('id'));

        $roleWaiter->permissions()->sync($permissions->whereIn('slug', TenantDefaultRolePermissions::waiter())->pluck('id'));

        $roleCleaning->permissions()->sync($permissions->whereIn('slug', TenantDefaultRolePermissions::cleaning())->pluck('id'));

        $roleGirl->permissions()->sync($permissions->whereIn('slug', TenantDefaultRolePermissions::girl())->pluck('id'));
