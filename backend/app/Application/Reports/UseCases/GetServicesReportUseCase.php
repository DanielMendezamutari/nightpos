...
final class GetServicesReportUseCase implements UseCaseInterface
{
    ...
    public function execute(?object $input = null): OperationResult
    {
        ...
        $data    = $this->reports->getServicesReport($tenant->id, $branch->id, $filters);

        return OperationResult::ok('Reporte de servicios.', $data);
    }
    ...
    private function extractFilters(?object $input): array
    {
        ...
        foreach (['date_from', 'date_to', 'official_shift_id', 'girl_user_id'] as $key) {
            if (isset($input->{$key})) {
