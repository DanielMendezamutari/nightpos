...
final class GetSettlementsReportUseCase implements UseCaseInterface
{
    ...
    public function execute(?object $input = null): OperationResult
    {
        ...
        $data    = $this->reports->getSettlementsReport($tenant->id, $branch->id, $filters);

        return OperationResult::ok('Reporte de liquidaciones.', $data);
    }
    ...
    private function extractFilters(?object $input): array
    {
        ...
        foreach (['date_from', 'date_to', 'official_shift_id'] as $key) {
            if (isset($input->{$key})) {
