...
final class GetProductReconciliationReportUseCase implements UseCaseInterface
{
    ...
    public function execute(?object $input = null): OperationResult
    {
        ...
        $data    = $this->reports->getProductReconciliation($tenant->id, $branch->id, $filters);

        return OperationResult::ok('Conciliación de productos.', $data);
    }
    ...
    private function extractFilters(?object $input): array
    {
        ...
        foreach (['date_from', 'date_to', 'official_shift_id', 'cash_session_id', 'waiter_user_id'] as $key) {
            if (isset($input->{$key})) {
