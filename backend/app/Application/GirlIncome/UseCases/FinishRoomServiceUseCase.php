...
final class FinishRoomServiceUseCase implements UseCaseInterface
{
    ...
    public function execute(?object $input = null): OperationResult
    {
        ...
        $releaseRoomImmediately = ! ($input instanceof \stdClass) || ! isset($input->releaseRoomImmediately)
            || (bool) $input->releaseRoomImmediately;
        ...
        if ($roomId !== null) {
            ...
            if ($releaseRoomImmediately) {
                $this->rooms->releaseAfterService($roomId, $tenant->id, $branch->id);
            } else {
                $this->rooms->setCleaning($roomId, $tenant->id, $branch->id);
            }
            ...
            $this->eventEmitter->emit(
                ...
                [
                    ...
                    'summary' => $releaseRoomImmediately
                        ? 'Habitación disponible: '.($entry['room_label'] ?? $entry['room_number'] ?? '')
                        : 'Habitación en limpieza: '.($entry['room_label'] ?? $entry['room_number'] ?? ''),
