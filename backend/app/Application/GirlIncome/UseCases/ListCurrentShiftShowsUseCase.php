...
final class ListCurrentShiftShowsUseCase implements UseCaseInterface
{
    ...
    public function execute(?object $input = null): OperationResult
    {
        ...
        $summary = $this->shows->summarizeForShift($tenant->id, $branch->id, $shift->id);

        return OperationResult::ok('Shows del turno actual.', [
            'shift' => GirlIncomeMapper::shift($shift),
            'summary' => [
                'total_amount' => number_format($summary['total_amount'], 2, '.', ''),
                'count' => $summary['count'],
            ],
            'items' => $this->shows->listForShift($tenant->id, $branch->id, $shift->id),
        ]);
    }
}
