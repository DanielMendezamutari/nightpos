...
const fmtBob = amount => formatMoney(amount ?? 0, 'BOB')
...
const pendingSummaryData = computed(() => {
  ...
  const scope = scopeSummaryData.value
  ...
  if (settlement) {
    return {
      ...
      critical_alerts: [...blockers.map(item => item.message), ...(scope.warnings ?? [])],
    }
  }
...
const expectedClosing = computed(() => toNumber(cashSummaryData.value.expected_cash))
...
const openCloseDialog = async () => {
  ...
  try {
    ...
    try {
      ...
      pendingSettlementsTotal.value = Number(data.summary?.total_pending ?? 0)
    }
    catch {
  ...
  <div class="cash-page">
    ...
    <template v-else-if="!session">
      ...
      >
        ...
        <div class="d-flex flex-wrap gap-2">
          <VBtn size="small" variant="tonal" prepend-icon="ri-file-text-line" @click="openCloseReceipt">
            Ver cierre
          </VBtn>
          <VBtn size="small" variant="tonal" prepend-icon="ri-printer-line" :loading="closeReprintLoading" @click="reprintCloseReceipt">
            Reimprimir cierre
          </VBtn>
        </div>
      </VAlert>
      ...
      <VCard max-width="520">
        <VCardText>
          ...
          <VBtn
            ...
            @click="showOpen = true"
          >
            <VIcon icon="ri-lock-unlock-line" start />
            Abrir caja
          </VBtn>
        </VCardText>
      </VCard>
    ...
    >
      <VCard title="Abrir caja">
        ...
        <VCardActions>
          <VBtn variant="text" @click="showOpen = false">
            Cancelar
          </VBtn>
          <VSpacer />
          <VBtn
    ...
    <VDialog
      ...
      max-width="520"
    >
      <VCard title="No puedes cerrar caja todavia">
        <VCardText>
          <VAlert
            type="error"
            variant="tonal"
        ...
        <VCardActions>
          <VSpacer />
          <VBtn variant="text" @click="showCloseBlockers = false">
            Entendido
          </VBtn>
        </VCardActions>
      </VCard>
    ...
    >
      <VCard title="Cerrar caja">
        <VCardText>
          <VAlert
            v-if="pendingSettlementsLoading"
            type="info"
            variant="tonal"
            class="mb-4"
          >
            Cargando pendientes de liquidaciones...
          </VAlert>
          ...
          >
            ...
            Si las pagas ahora, se descontaran de tu caja.
            <VBtn
              size="small"
              variant="text"
              color="warning"
        ...
        <VCardActions>
          <VBtn variant="text" @click="showClose = false">
            Cancelar
          </VBtn>
          <VSpacer />
          <VBtn
