    ...
    $("#busquedaproducto").keypress(function(e) {
        if (e.charCode == 13 || e.keyCode == 13) { //ENTER
            ...
            if (code == "") {
                ...
                var SubtotalDiscriminado = parseFloat(Operac3) - parseFloat(Discriminado.toFixed(2));
...
$(document).ready(function(){
    ...
    if ($('#cobrarmesa input#descuento').val() > 100) {
        ...
        $("#montodevuelto").val((TotalGeneral == sumpagado) ? "0.00" : original_cambio.toFixed(2));

        $("#TotalImporte").text(Separador(TotalGeneral.toFixed(2)));
        $("#TotalPagado").text(Separador(sumpagado.toFixed(2)));
        $("#TextCambio").text((TotalGeneral == sumpagado) ? "0.00" : Separador(original_cambio.toFixed(2)));
    });
});
...
function MostrarDetallesPedidos(codmesa,codsucursal){

$('#muestradetallepedidosmesa').html('<center><i class="fa fa-spin fa-spinner"></i> Cargando información, por favor espere....</center>');

var dataString = "BuscaDetallesPedidosModal=si&codmesa="+codmesa+"&codsucursal="+codsucursal;
...
function EliminaPedidoMesa(coddetallepedido,codpedido,codmesa,codsucursal,tipo) {
        swal({
          ...
          confirmButtonColor: "#3085d6"
            ...
            $('#muestradetallepedidosmesa').load("detalles_mesas.php?BuscaDetallesPedidosModal=si&codmesa="+codmesa+"&codsucursal="+codsucursal);
            }
        })
    });
}
...
if (opcionpropina == 1) {
    ...
    $("#TotalImporte").text(Separador(Sumatoria.toFixed(2)));
    $("#TotalPagado").text(Separador(sumpagado.toFixed(2)));
    $("#TextCambio").text((Sumatoria == sumpagado) ? "0.00" : Separador(original_cambio.toFixed(2)));
    $("#montodevuelto").val((Sumatoria == sumpagado) ? "0.00" : original_cambio.toFixed(2));
}
