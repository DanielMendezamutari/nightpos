  ...
  try {
    ...
    if (addForm.allocations?.length) {
      if (!addedAllocationTarget)
        throw new Error('No se pudo identificar la línea de combo recién agregada para sincronizar reparto.')
    ...
    const pending = addedAllocationTarget
      ? (order.value?.items ?? [])
      .find(i => Number(i?.id) === Number(addedAllocationTarget?.id) && i.requires_allocation && !i.allocation_complete)
      : null
