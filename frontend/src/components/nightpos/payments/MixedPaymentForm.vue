...
const props = defineProps({
  ...
  /** selector = dropdown (comandas); inline = campos siempre visibles + atajos (venta directa) */
  variant: { type: String, default: 'selector' },
...
const {
  ...
  validate,
  toPayload,
} = useMixedPayments(totalRef, { variant: props.variant })
...
<template>
  <div class="mixed-payment-form">
    ...
    <template v-if="variant === 'inline' || form.method === 'MIXED'">
      <p
        v-if="variant === 'inline'"
      ...
      >
        Suma pagos: {{ paymentsSum.toFixed(2) }} / {{ total.toFixed(2) }} {{ currency }}
      </p>
    </template>

    <div
      v-if="showQuickButtons && variant === 'inline'"
